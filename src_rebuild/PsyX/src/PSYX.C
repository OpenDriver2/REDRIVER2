...
static int GR_InitialiseSDL(char* windowName, int width, int height, int fullscreen)
{
	g_windowWidth  = width;
	...
	if (GR_InitialiseGLContext(windowName, fullscreen) == FALSE)
	{
	...
	if (GR_InitialiseGLESContext(windowName, fullscreen) == FALSE)
	{
...
void PsyX_Initialise(char* windowName, int width, int height, int fullscreen)
{
	...
	if (GR_InitialiseSDL(windowName, width, height, fullscreen) == FALSE)
	{
...
void PsyX_Sys_DoDebugKeys(int nKey, bool down)
{
	...
	if (!down)
	{
		...
		case SDL_SCANCODE_F10:
			...
			GR_SaveVRAM("VRAM.TGA", 0, 0, VRAM_WIDTH, VRAM_HEIGHT, TRUE);
			break;
#endif
#if !defined(__EMSCRIPTEN__) && !defined(__ANDROID__)
